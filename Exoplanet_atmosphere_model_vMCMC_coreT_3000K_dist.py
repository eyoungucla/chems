...
#0-24 (i.e., 1-25) are the mole fractions, and the last three
...
print('Opening initial_atm_vMCMC.txt...')
name="initial_atm_vMCMC.txt"
...
for i in range(0,numvar-1):
    var[i]=values[i]
var[numvar-1]=values[30]
    
# Specify mass of planet
Mplanet_Mearth=values[28]  #Planet mass in Earth masses

# Create array of temperatures
T_min = 1300.0  #K
T_max = 3000.0 #K THIS DEFINES CORE-MANTLE BOUNDARY T, as T(num-1) or as T_max
TK=np.linspace(T_min,T_max,num)

#select temperature for optimization and find nearest value in array TK
temp=values[29]
print('Selected temperature=',temp,'K')
...
wt_massbalance=values[31]
print('Weight for mass balance equations =', wt_massbalance)

#Assign weight for summing constraints
wt_summing=values[32]
print('Weight for summing constraints =', wt_summing)

#Assign weights to groups of reactions
watm_m=values[33]  #Atmosphere rxns
wsolub_m=values[34] #solubility
wmelt_m=values[35]  #intra-melt reactions
wevap_m=values[36] #evaporation reactions
...
rxn_names=[' ','Na2SiO3 = Na2O + SiO2','1/2SiO2 + Fe = FeO + 1/2Si','MgSiO3 = MgO + SiO2','O + 1/2Si = 1/2SiO2',\
    '2Hmetal=H2,sil','FeSiO3 = FeO + SiO2','2H2O,sil + Si=SiO2 + 2H2,sil','CO,g + 1/2O2 = 2H2,g + CO2,g','CH4,g + 1/2O2 = 2H2,g + CO,g',\
    'H2,g + 1/2O2 = H2O,g','FeO = Fe,g + 1/2O2','MgO = Mg,g + 1/2O2','SiO2 = SiO,g + 1/2O2','Na2O = 2Na,g + 1/2O2',\
    'H2,g = H2,sil','H2O,g = H2O,sil','CO,g = CO,sil','CO2,g = CO2,sil','Si','Mg','O','Fe','H','Na','C','sum xi melt',\
...
var_names=['xMgO melt','xSiO2 melt','xMgSiO3 melt','xFeO melt','xFeSiO3 melt','xNa2O melt','xNa2SiO3 melt','xH2 melt','xH2O melt', \
    ...
    'xH2O gas','xFe gas','xMg gas','xSiO gas','xNa gas','Moles atm','Moles silicate','Moles metal',' Pressure weighting']
print('')
...
grams_per_mole_atm=var[15]*mol_wts[15]+var[16]*mol_wts[16]+\
    ...
    var[23]*mol_wts[23]+var[24]*mol_wts[24]
...
while i < num-1:
    if TK[i] < 1700.0:
        ...
        g=223.3967
        h=-241.8264
        HgasH2O=-241.83+a*ti+(b*ti**2.0)/2.0+(c*ti**3.0)/3.0+(d*ti**4.0)/4.0-(e/ti)+f-h
        HgasH2O=HgasH2O*1000.0 #convert kJ to J
        SgasH2O=a*ln(ti)+b*ti+(c*ti**2.0)/2.0+(d*ti**3.0)/3.0-(e/(2.0*ti**2.0))+g
        GgasH2O=HgasH2O-TK*SgasH2O  #apparent G of formation for H2O gas at T and 1 bar
    else:
        ti=TK[i]/1000.0
        a=41.96426
        b=8.622053
...
while i < num-1:
    if TK[i] < 1200.0:
        ...
        g=228.2431
        h=-393.5224
        HgasCO2=-393.51+a*ti+(b*ti**2.0)/2.0+(c*ti**3.0)/3.0+(d*ti**4.0)/4.0-(e/ti)+f-h
        HgasCO2=HgasCO2*1000.0 #convert kJ to J
        SgasCO2=a*ln(ti)+b*ti+(c*ti**2.0)/2.0+(d*ti**3.0)/3.0-(e/(2.0*ti**2.0))+g
        GgasCO2=HgasCO2-TK*SgasCO2 #apparent G of formation of CO2 gas at T and 1 bar
    else:
        ti=TK[i]/1000.0
        a=58.16639
        b=2.720074
...
while i < num-1:
    if TK[i] < 1300.0:
        ...
        g=227.3665
        h=-110.5271
        HgasCO=-110.53+a*ti+(b*ti**2.0)/2.0+(c*ti**3.0)/3.0+(d*ti**4.0)/4.0-(e/ti)+f-h
        HgasCO=HgasCO*1000.0 #convert kJ to J
        SgasCO=a*ln(ti)+b*ti+(c*ti**2.0)/2.0+(d*ti**3.0)/3.0-(e/(2.0*ti**2.0))+g
        GgasCO=HgasCO-TK*SgasCO #apparent G of formation of CO gas at T and 1 bar
    else:
        ti=TK[i]/1000.0
        a=35.15070
        b=1.300095
...
while i < num-1:
    if TK[i] < 1300.0:
        ...
        g=158.7163
        h=-74.87310
        HgasCH4=-74.873+a*ti+(b*ti**2.0)/2.0+(c*ti**3.0)/3.0+(d*ti**4.0)/4.0-(e/ti)+f-h
        HgasCH4=HgasCH4*1000.0 #convert kJ to J
        SgasCH4=a*ln(ti)+b*ti+(c*ti**2.0)/2.0+(d*ti**3.0)/3.0-(e/(2.0*ti**2.0))+g
        GgasCH4=HgasCH4-TK*SgasCH4 #apparent G of formation of CO gas at T and 1 bar
    else:
        ti=TK[i]/1000.0
        a=85.81217
        b=11.26467
...
while i < num-1:
    if TK[i] < 1100.0:
        ...
        SgasSiO=a*ln(ti)+b*ti+(c*ti**2.0)/2.0+(d*ti**3.0)/3.0-(e/(2.0*ti**2.0))+g
...
while i < num-1:
    if TK[i] < 2200.0:
        ...
        SgasMg=a*ln(ti)+b*ti+(c*ti**2.0)/2.0+(d*ti**3.0)/3.0-(e/(2.0*ti**2.0))+g
...
#
nSi=var[26]*(var[1]+var[2]+var[4]+var[6])+var[12]*var[27]+var[23]*var[25]
nMg=var[26]*(var[0]+var[2])+var[22]*var[25]
nO=var[26]*(var[0]+2.0*var[1]+3.0*var[2]+var[3]+3.0*var[4]+var[5]+3.0*var[6]+var[8]+var[9]+2.0*var[10]) \
    + var[27]*var[13]+var[25]*(var[23]+var[9]+2.0*var[17]+2.0*var[19]+var[20])
nFe=var[26]*(var[3]+var[4])+var[27]*var[11]+var[25]*var[21]
nH=var[27]*var[14]+var[26]*(2.0*var[7]+2.0*var[8])+var[25]*(2.0*var[15]+2.0*var[20]+4.0*var[18])
nNa=var[26]*(2.0*var[5]+2.0*var[6])+var[25]*var[24]
nC=var[26]*(var[9]+var[10])+var[25]*(var[18]+var[17]+var[16])
...
melt_min=0.5 #0.9
melt_max=2.0  #1.1
bounds[25,0]=var[25]*1.0e-20
bounds[25,1]=var[25]*50.0
bounds[26,0]=var[26]*melt_min
bounds[26,1]=var[26]*melt_max
bounds[27,0]=var[27]*0.5
bounds[27,1]=var[27]*2.0
...
GRT1_T=GRT1[nT]
#GRT2_T=GRT2[nT]
GRT2_T=GRT2[num-1] # proxy for core-mantle boundary using maximum T as T
GRT3_T=GRT3[nT]
#GRT4_T=GRT4[nT]
GRT4_T=GRT4[num-1] # proxy for core-mantle boundary using maximum T as T
#GRT5_T=GRT5[nT]
GRT5_T=GRT5[num-1] # proxy for core-mantle boundary using maximum T as T
GRT6_T=GRT6[nT]
#GRT7_T=GRT7[nT]
GRT7_T=GRT7[num-1] # proxy for core-mantle boundary using maximum T as T
GRT8_T=GRT8[nT]
GRT9_T=GRT9[nT]
GRT10_T=GRT10[nT]
GRT11_T=GRT11[nT]
...
GRT12_T,GRT13_T,GRT14_T,GRT15_T,GRT16_T,GRT17_T,GRT18_T,nSi,nMg,nO,nFe,nH,nNa,nC)
...
f19_ini=nSi-(var[26]*(var[1]+var[2]+var[4]+var[6])+var[12]*var[27]+var[23]*var[25])
f20_ini=nMg-(var[26]*(var[0]+var[2])+var[22]*var[25])
f21_ini=nO-(var[26]*(var[0]+2.0*var[1]+3.0*var[2]+var[3]+3.0*var[4]+var[5]+3.0*var[6]+var[8]+var[9]+2.0*var[10]) \
        + var[27]*var[13]+var[25]*(var[23]+var[9]+2.0*var[17]+2.0*var[19]+var[20]) )
f22_ini=nFe-(var[26]*(var[3]+var[4])+var[27]*var[11]+var[25]*var[21])
f23_ini=nH-(var[27]*var[14]+var[26]*(2.0*var[7]+2.0*var[8])+var[25]*(2.0*var[15]+2.0*var[20]+4.0*var[18]))
f24_ini=nNa-(var[26]*(2.0*var[5]+2.0*var[6])+var[25]*var[24])
f25_ini=nC-(var[26]*(var[9]+var[10])+var[25]*(var[18]+var[17]+var[16]))

#Summing constraint on mole fractions
f26_ini=1.0-var[0]-var[1]-var[2]-var[3]-var[4]-var[5]-var[6]-var[7]-var[8]-var[9]-var[10]
f27_ini=1.0-var[11]-var[12]-var[13]-var[14]
f28_ini=1.0-var[15]-var[16]-var[17]-var[18]-var[19]-var[20]-var[21]-var[22]-var[23]-var[24]
...
sum_thermo_i=sum_thermo_i+abs(f11_ini)+abs(f12_ini)+abs(f13_ini)+abs(f14_ini)+abs(f15_ini)+abs(f16_ini)+abs(f17_ini)+abs(f18_ini)
...
print('Scaling for f1 through f18 =',wgas)
print('Scaling for f19 through f25 =',wtn)
print('Scaling for f26 through f28 =',wtx)
...
#    mult=max_f
    ...
    wtm=5.0
    f19=fm[19]*(wtm/nSi)*wtn*(nSi-(var[26]*(var[1]+var[2]+var[4]+var[6])+var[12]*var[27]+var[23]*var[25]))
    f20=fm[20]*(wtm/nMg)*wtn*(nMg-(var[26]*(var[0]+var[2])+var[22]*var[25]))
    f21=fm[21]*(wtm/nO)*wtn*(nO-(var[26]*(var[0]+2.0*var[1]+3.0*var[2]+var[3]+3.0*var[4]+var[5]+3.0*var[6]+var[8]+var[9]+2.0*var[10]) \
        + var[27]*var[13]+var[25]*(var[23]+var[9]+2.0*var[17]+2.0*var[19]+var[20])))
    f22=fm[22]*(wtm/nFe)*wtn*(nFe-(var[26]*(var[3]+var[4])+var[27]*var[11]+var[25]*var[21]))
    f23=fm[23]*(wtm/nH)*wtn*(nH-(var[27]*var[14]+var[26]*(2.0*var[7]+2.0*var[8])+var[25]*(2.0*var[15]+2.0*var[20]+4.0*var[18])))
    f24=fm[24]*(5.0/nNa)*wtn*(nNa-(var[26]*(2.0*var[5]+2.0*var[6])+var[25]*var[24]))
    f25=fm[25]*(5.0/nC)*wtn*(nC-(var[26]*(var[9]+var[10])+var[25]*(var[18]+var[17]+var[16])))
    ...
    f26=fm[26]*wtx*(1.0-var[0]-var[1]-var[2]-var[3]-var[4]-var[5]-var[6]-var[7]-var[8]-var[9]-var[10])
    f27=fm[27]*wtx*(1.0-var[11]-var[12]-var[13]-var[14])
    f28=fm[28]*wtx*(1.0-var[15]-var[16]-var[17]-var[18]-var[19]-var[20]-var[21]-var[22]-var[23]-var[24])
    ...
    f26=f26*(1.0+f26_penalty*mult) #preserves the shape
...
#Sum of squared errors
    ...
    sum1=sum1+f11**2.0+f12**2.0+f13**2.0+f14**2.0+f15**2.0+f16**2.0+f17**2.0+f18**2.0
    sum2=f19**2.0+f20**2.0+f21**2.0+f22**2.0+f23**2.0+f24**2.0+f25**2.0
    sum3=(f26**2.0+f27**2.0+f28**2.0)
    sum4=f29**2.0
...
def model(theta):
    ...
    y_model[18]=(theta[26]*(theta[1]+theta[2]+theta[4]+theta[6])+theta[12]*theta[27]+theta[23]*theta[25])
    y_model[19]=(theta[26]*(theta[0]+theta[2])+theta[22]*theta[25])
    y_model[20]=(theta[26]*(theta[0]+2.0*theta[1]+3.0*theta[2]+theta[3]+3.0*theta[4]+theta[5]+3.0*theta[6]+theta[8]+theta[9]+2.0*theta[10]) \
        + theta[27]*theta[13]+theta[25]*(theta[23]+theta[9]+2.0*theta[17]+2.0*theta[19]+theta[20]))
    y_model[21]=(theta[26]*(theta[3]+theta[4])+theta[27]*theta[11]+theta[25]*theta[21])
    y_model[22]=(theta[27]*theta[14]+theta[26]*(2.0*theta[7]+2.0*theta[8])+theta[25]*(2.0*theta[15]+2.0*theta[20]+4.0*theta[18]))
    y_model[23]=(theta[26]*(2.0*theta[5]+2.0*theta[6])+theta[25]*theta[24])
    y_model[24]=(theta[26]*(theta[9]+theta[10])+theta[25]*(theta[18]+theta[17]+theta[16]))
    #Model summing constraint on mole fractions, should sum to unity
    y_model[25]=(theta[0]+theta[1]+theta[2]+theta[3]+theta[4]+theta[5]+theta[6]+theta[7]+theta[8]+theta[9]+theta[10])
    y_model[26]=(theta[11]+theta[12]+theta[13]+theta[14])
    y_model[27]=(theta[15]+theta[16]+theta[17]+theta[18]+theta[19]+theta[20]+theta[21]+theta[22]+theta[23]+theta[24])
    ...
    y_model[28]=100.0*(P_guess-theta[28])/P_guess #Added a multiplier here to force better pressure solutions
...
def main(p0,nwalkers,niter,n,lnprob,data):
    ...
    with Pool(5,context=ctx) as pool:
...
print('')
nSi_final=result[26]*(result[1]+result[2]+result[4]+result[6])+result[12]*result[27]+result[23]*result[25]
nMg_final=result[26]*(result[0]+result[2])+result[22]*result[25]
nH_final=result[27]*result[14]+result[26]*(2.0*result[7]+2.0*result[8])+result[25]* \
    (2.0*result[15]+2.0*result[20]+4.0*result[18])
nO_final=result[26]*(result[0]+2.0*result[1]+3.0*result[2]+result[3]+3.0*result[4]+result[5]+ \
    3.0*result[6]+result[8]+result[9]+2.0*result[10])+result[27]*result[13]+result[25]*(result[23]+ \
    result[9]+2.0*result[17]+2.0*result[19]+result[20])
nFe_final=result[26]*(result[3]+result[4])+result[27]*result[11]+result[25]*result[21]
nNa_final=result[26]*(2.0*result[5]+2.0*result[6])+result[25]*result[24]
nC_final=result[26]*(result[9]+result[10])+result[25]*(result[18]+result[17]+result[16])
...
for i in range(0,18):
    ...
    K[i]=exp(best_fit_model[i])  # Calculated equilibrium constants from mole fractions
...
f21_f=nO-(result[26]*(result[0]+2.0*result[1]+3.0*result[2]+result[3]+3.0*result[4]+result[5]+3.0*result[6]+result[8]+result[9]+2.0*result[10]) \
        + result[27]*result[13]+result[25]*(result[23]+result[9]+2.0*result[17]+2.0*result[19]+result[20]) )
...
a_file = open('output_summary_atm.txt', 'w')
...
#'xH2Og','xFeg','xMgg','xSiOg','xNag','Matm','Msil','Mmetal',' Press']
#labels_short=labels[12:16] #replaces labels
#smpl_test=samples[:,12:16] #replaces samples
